  ...
  return (
    <div className="p-4 sm:p-6 w-full space-y-5">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-3">
        ...
        <div className="bg-dark-700 rounded-xl border border-dark-300 p-4 text-center">
          <div className="text-2xl font-bold text-accent">{users.reduce((s, u) => s + (u.credits || 0), 0)}</div>
          <div className="text-xs text-slate-400">Кредитов всего</div>
        </div>
        <div className="bg-dark-700 rounded-xl border border-dark-300 p-4 text-center">
        ...
        <div className="bg-dark-700 rounded-xl border border-dark-300 p-4 text-center">
          <div className="text-2xl font-bold text-warn">{users.filter(u => u.credits === 0).length}</div>
          <div className="text-xs text-slate-400">Без кредитов</div>
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-xs text-slate-500 border-b border-dark-300">
                  ...
                  <th className="text-right px-4 py-2.5 font-medium">Вакансий</th>
                ...
                {users.map(u => (
                  <tr key={u.id} className={`hover:bg-dark-600 transition ${expandedUser === u.id ? 'bg-dark-600/50' : ''}`}>
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <span className={`font-semibold ${u.credits > 0 ? 'text-accent' : 'text-danger'}`}>
                        {u.credits}
                      </span>
                    </td>
                    <td className="px-4 py-2.5 text-right">
                    ...
                    <td className="px-4 py-2.5 text-right text-slate-400">{u.total_vacancies || 0}</td>
                    <td className="px-4 py-2.5 text-slate-500 text-xs">{formatDate(u.created_at)}</td>
                    <td className="px-4 py-2.5">
                      {u.is_admin ? (
                        <span className="px-2 py-0.5 rounded bg-accent/20 text-accent text-xs font-medium">admin</span>
                      ) : (
                    ...
                    <td className="px-4 py-2.5 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          className="px-2 py-1 text-xs bg-success/20 text-success rounded-lg hover:bg-success/30 transition"
